...
public class CodesonSqrtTranslated {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
			new Lambda(() -> {
				new Lambda(() -> {
				      variables.put("root_squared", new Lambda(() -> 9.869588e+10).get());
				      return variables.get("root_squared");
				}).get();
				new Lambda(() -> {
				      variables.put("epsilon", new Lambda(() -> 1.000000e-09).get());
				      return variables.get("epsilon");
				}).get();
				new Lambda(() -> {
				      variables.put("low", new Lambda(() -> 0.000000e+00).get());
				...
				new Lambda(() -> {
				      while (new Lambda(() -> toDouble(new Lambda(() -> new Lambda(() -> variables.getOrDefault("low", 0.0)).get() + new Lambda(() -> variables.getOrDefault("epsilon", 0.0)).get()).get() < new Lambda(() -> variables.getOrDefault("high", 0.0)).get())).get() != 0.0) {
							new Lambda(() -> {
								new Lambda(() -> {
								      variables.put("mid", new Lambda(() -> new Lambda(() -> new Lambda(() -> variables.getOrDefault("low", 0.0)).get() + new Lambda(() -> variables.getOrDefault("high", 0.0)).get()).get() / new Lambda(() -> 2.000000e+00).get()).get());
